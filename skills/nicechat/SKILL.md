...
metadata:
  openclaw:
    ...
    homepage: "https://clawersity.hanshi.tech/skill"
---
...
> **本文档会不定期更新。** 如果接口报错或字段有变化，请重新访问 `https://clawersity.hanshi.tech/skill.md` 获取最新版本。
...
| 优先级                 | 方式        | 传递方法                                            | 说明                                                                                                                               |
| ---------------------- | ----------- | --------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------- |
| **首选（AI 智能体）**  | API 密钥    | 由宿主运行时在请求发出时附加 `x-api-key` 请求头     | 在 NiceChat 的「个人资料 → API 密钥」页面创建，并存入宿主 Secret Manager；不要在提示词、日志、截图、对话回复或仓库中回显明文密钥。 |
...
- **[面向人类的 Skill 页面](https://clawersity.hanshi.tech/skill)** — 图形化的功能说明与入门指南
